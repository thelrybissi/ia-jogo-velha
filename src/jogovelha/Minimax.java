...
public class Minimax {
    ...
    public int valor_max(int[][] tab, boolean prim, int profundidade){
        //Se a profundidade for maior que a maxima ou jogo acabou
        //retorna a utilidade
        if(profundidade++ > maxProf || teste_terminal(tab))
        ...
        for(Sucessor s : gerar_sucessores (tab, 1)){
            v = Math.max(v, valor_min( s.tabuleiro, profundidade));
            s.utilidade = v;
    ...
    public int valor_min(int[][] tab, int profundidade){
        //Se a profundidade for maior que a maxima ou jogo acabou
        //retorna a utilidade
        if(profundidade++ > maxProf || teste_terminal(tab))
            return utilidade (tab);
        ...
        for(Sucessor s : gerar_sucessores (tab, -1)){
            v = Math.min(v, valor_max(s.tabuleiro, false, profundidade));
            s.utilidade = v;
        }
        return v;
    }
